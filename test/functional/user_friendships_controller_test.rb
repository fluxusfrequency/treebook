...
class UserFriendshipsControllerTest < ActionController::TestCase
  ...
  context "when logged in" do
    setup do
      @pending_friendship = create(:pending_user_friendship, user: users(:ben), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
      @accepted_friendship = create(:accepted_user_friendship, user: users(:ben), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
      @requested_friendship = create(:requested_user_friendship, user: users(:ben), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
      @blocked_friendship = user_friendships(:blocked_by_ben)
    ...
    context "accepted friendships" do
      ...
      should "not display pending or requested friend's names" do
        assert_select "div#friend-list", {count: 1, html: /#{@accepted_friendship.friend.full_name}/ }
        assert_select "div#friend-list", {count: 0, html: /#{@blocked_friendship.friend.full_name}/ }
      end

      should "display active friend names" do
        assert_match /Active/, response.body
      end
    end
    ...
    context "when logged in" do
      ...
      context "with a valid friend_id" do
        ...
        should "create a friendship" do
          assert users(:ben).pending_friends.include?(users(:fred))
        end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:ben), friend: @friend)
        create(:pending_user_friendship, friend: users(:ben), user: @friend)
        sign_in users(:ben)
  ...
  context "#edit" do
    ...
    context "when logging in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:ben))
        sign_in users(:ben)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:ben))
        create(:accepted_user_friendship, friend: users(:ben), user: @friend)
